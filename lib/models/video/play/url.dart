...
class PlayUrlModel {
  ...
  int findAvailableVideoQuality(int preferredQuality) {
    final qualities = dash!.video!.availableVideoQualities;
    if (qualities.isEmpty) {
      throw StateError('No video qualities are available');
    }
    return qualities.where((e) => e <= preferredQuality).reduceOrNull(max) ??
        qualities.reduce(max);
  }
